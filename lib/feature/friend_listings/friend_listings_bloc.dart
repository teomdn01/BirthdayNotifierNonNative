...
class FriendListingsBloc extends Bloc<FriendListingsEvent, FriendListingsState>{
  final FriendRepository _friendRepository = GetIt.instance.get<FriendRepository>();

  FriendListingsBloc() : super(const FriendListingsInitial());
  @override
  Stream<FriendListingsState> mapEventToState(FriendListingsEvent event) async*{
    if (event is getFriendListings){
        yield const FriendListingsLoading();
        final List<FriendListing> listings = await _friendRepository.fetchFriends();
